...
const educationData = [
  {
    ...
    tech: ["Java", "Spring Boot", "React", "Node.js", "MongoDB"],
    icon: GraduationCap,
...
// memo: avoids re-renders since all data is module-level constants
const PremiumExperienceCard = memo(function PremiumExperienceCard() {
  ...
  return (
    <motion.div
      ...
      // ✅ FIX: whileHover removed from the outer wrapper — it was triggering
      //    layout recalculation for the entire card (including all children)
      //    on every hover. CSS transform is used instead via the group pattern.
      className="group relative w-full rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 overflow-hidden shadow-2xl shadow-black/30 ring-1 ring-white/10 transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[5px]"
    >
      {/* Decorative blobs — pointer-events:none, no repaints */}
      ...
      <div className="relative p-5 sm:p-6 md:p-7 lg:p-8">
        ...
        <div className="flex flex-col md:flex-row md:flex-wrap justify-between items-start gap-4 mb-6">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden bg-white flex-shrink-0 ring-2 ring-white/20 shadow-lg">
              ...
              <div>
                <h3 className="text-1xl md:text-2xl lg:text-2xl font-bold text-white tracking-tight leading-tight">
          ...
          {/*
           * ✅ FIX: Replaced motion.button (whileHover/whileTap) with a plain
           *    <button> using CSS transforms. Each Framer Motion whileHover
           *    subscribes a pointer-enter listener AND schedules a JS animation
           *    — for a simple scale this is unnecessary overhead.
           */}
          <button
            onClick={handleVerifyExperience}
            className="relative overflow-hidden inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-900/80 backdrop-blur-md border border-white/20 text-xs font-semibold text-white/90 hover:text-white transition-all duration-300 group/btn shadow-md active:scale-[0.97] hover:scale-[1.04]"
        ...
        {/* Bullet Points
         * ✅ FIX: Removed motion.span whileHover on every bullet — that was
         *    creating 5 Framer Motion animation subscribers just for the icon.
         *    CSS hover + transition achieves the identical visual at zero JS cost.
         */}
        <div className="space-y-2 mb-6">
          {experienceData.descBullets.map((point, idx) => (
            ...
            >
              <span className="text-purple-400 text-base mt-0.5 group-hover:text-purple-300 group-hover:drop-shadow-[0_0_6px_rgba(168,85,247,0.5)] transition-all duration-300 select-none">
                ✦
              </span>
        ...
        {/* Tech Stack — CSS tooltip, no JS state */}
...
// ==================== EDUCATION CARD ====================
// memo: each card only depends on its own edu object (stable reference)
const EducationCard = memo(function EducationCard({
  ...
  return (
    <motion.div
      ...
      // ✅ FIX: whileHover → CSS transition (same visual, zero JS animation cost)
      className="group relative rounded-2xl overflow-hidden transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[6px]"
      style={{
    ...
    >
      ...
      {/* Top accent line */}
      <div
        className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${edu.accentFrom} ${edu.accentTo} opacity-70 group-hover:opacity-100 transition-opacity duration-300`}
      />
      ...
      <div className="relative p-6 sm:p-7">
        ...
        <div className="flex items-center justify-between mb-5">
          <span
            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold border backdrop-blur-sm ${edu.badgeColor}`}
          >
          ...
          <div className="flex flex-col items-end">
            <span
              className={`text-2xl font-bold bg-gradient-to-r ${edu.accentFrom} ${edu.accentTo} bg-clip-text text-transparent leading-none`}
            >
              {edu.score}
            </span>
        ...
        <div className="flex items-start gap-4 mb-4">
          <div
            className={`w-11 h-11 rounded-xl bg-gradient-to-br ${edu.accentFrom} ${edu.accentTo} flex items-center justify-center shadow-lg flex-shrink-0 group-hover:scale-110 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]`}
          >
...
/*
 * ✅ FIX: The original file injected a <style> tag via document.createElement
 *    at MODULE PARSE TIME (top-level code, outside any function or hook).
 *    This ran synchronously on import — before React even started rendering —
 *    and added a forced style recalculation to the critical path.
 *
 *    The shimmer keyframe is defined in index.css instead, which is already
 *    loaded as part of the initial CSS bundle and parsed by the browser before
 *    any JS runs — zero extra cost.
 *
 *    Add this to index.css once:
 *    @keyframes shimmer {
